...
export class UserController {
  ...
  @Post("/login") async Login(
    @Res() response,
    @Body() userLoginRequest: UserLoginRequestDTO,
  ) {
    await handle(response, async () => {
      ...
      if (
        user !== undefined &&
        user !== null &&
        (session = await this.userService.authenticate(
  ...
  @Post("/changeRole") async ChangeRole(
    @Res() response,
    @Query("token") token,
    @Body() dto: UserChangeRoleRequestDTO,
  ) {
    await handleAuth(
      response,
      this.userService,
      ...
      async (user) => {
        if (dto.user.username !== user.username) {
          let update = await this.userService.update(
            dto.user.username,
            dto.role,
          );
  ...
  @Get("/auth?") async Auth(@Res() response, @Query("token") token) {
    await handle(response, async () => {
      ...
      if (
        ...
        userSession !== null &&
        (await this.userService.verify(userSession.session))
      ) {
        return {
          data: {
            success: true,
